import { X, MapPin, Eye, Heart, Phone, Calendar, Building2, Car, Maximize2, Layers, BadgeCheck, Share2, ArrowUpRight, FileText, ExternalLink, ChevronDown, ChevronUp } from "lucide-react";
...
const PropertyDetailPanel = ({ property, onClose }: PropertyDetailPanelProps) => {
  ...
  // 세움터 건축물대장 열람 URL (주소 기반)
  const eaisUrl = `https://www.eais.go.kr/molit/com/msg/selectMolitSiteMsg.do`;
          ...
          {buildingOpen && (
            <div className="mt-2 rounded-xl border border-border overflow-hidden">
              ...
              <div className="px-3 py-3 grid grid-cols-2 gap-y-2 gap-x-4 bg-white text-xs">
                <div>
                  <p className="text-[10px] text-muted-foreground">건물 유형</p>
                  <p className="font-semibold text-foreground">{property.type}</p>
                </div>
                <div>
                  <p className="text-[10px] text-muted-foreground">준공연도</p>
                  <p className="font-semibold text-foreground">{property.buildYear}</p>
                </div>
                <div>
                  <p className="text-[10px] text-muted-foreground">총 층수</p>
                  <p className="font-semibold text-foreground">{property.totalFloors}</p>
                </div>
                <div>
                  <p className="text-[10px] text-muted-foreground">엘리베이터</p>
                  <p className="font-semibold text-foreground">{property.elevator ? "있음" : "없음"}</p>
                </div>
              </div>
              ...
              <div className="px-3 py-2.5 bg-muted/20 border-t border-border flex flex-col gap-2">
                ...
                <div className="flex gap-2">
                  <a
                    href={buildingSearchUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex-1 flex items-center justify-center gap-1.5 h-8 rounded-lg bg-primary text-white text-[11px] font-bold hover:bg-primary/90 transition-colors"
                  >
                    <ExternalLink className="w-3 h-3" />
                    세움터 열람
                  </a>
                  <a
                    href={naverBuildingUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex-1 flex items-center justify-center gap-1.5 h-8 rounded-lg bg-accent text-white text-[11px] font-bold hover:bg-accent/90 transition-colors"
                  >
                    <ExternalLink className="w-3 h-3" />
                    네이버 건물정보
                  </a>
                ...
                <p className="text-[10px] text-muted-foreground leading-relaxed">
                  세움터(eais.go.kr)에서 건축물대장 전체 내용을 무료로 열람하실 수 있습니다.
                </p>
              </div>
            </div>
          )}
