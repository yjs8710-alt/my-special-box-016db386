...
const LandlordSearchModal = ({ onClose }: LandlordSearchModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-[10100] flex items-center justify-center bg-black/50 backdrop-blur-sm px-4" onClick={onClose}>
      <div className="bg-card rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-5 pt-4 pb-3">
          ...
          {searched && !loading && (
            <p className="text-[10px] text-muted-foreground mt-1.5 pl-1">매물(숨김 포함) + 청주 연락처DB 전체 통합 검색</p>
          )}
        </div>
            ...
            return (
              <div key={item.id}
                ...
                <div className="border-t border-border/50 pt-2 flex flex-col gap-1.5">
                  {item.contactOwner ? (
                    <PhoneRow label="소유주(임대인)" phone={item.contactOwner} color="hsl(var(--primary))" show={show} onReveal={reveal} />
                  ) : (
                    <div className="flex items-center gap-1.5">
                  ...
                  {item.contactManager && (
                    <PhoneRow label="관리인" phone={item.contactManager} color="hsl(var(--chart-4))" show={show} onReveal={reveal} />
                  )}
                  {item.contactBroker && (
                    <PhoneRow label="부동산" phone={item.contactBroker} color="hsl(var(--chart-3))" show={show} onReveal={reveal} />
                  )}
                </div>
              </div>
            );
