...
const NotificationBell = ({ variant = "desktop" }: Props) => {
  ...
  return (
    <button
      onClick={() => navigate(targetPath)}
      className="relative inline-flex items-center justify-center p-1 overflow-visible"
      aria-label="알림"
    >
      <img src={iconBellNeon} alt="알림" className="w-16 h-16 object-contain" />
      {count > 0 && (
        <span className="absolute top-1 right-1 min-w-[16px] h-4 px-1 rounded-full text-[10px] font-bold flex items-center justify-center bg-destructive text-destructive-foreground shadow">
          {count > 99 ? "99+" : count}
        </span>
      )}
    </button>
