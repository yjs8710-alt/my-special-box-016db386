...
const ContactEmojiRow = forwardRef<HTMLDivElement, ContactEmojiRowProps>(
  ({ propId, type, number }, ref) => {
    const label =
      type === "owner"
        ? "소유주"
        : type === "tenant"
    ...
    const [revealed, setRevealed] = useState(
      () => !!number && hasRevealedToday(propId, type)
    );
    const [showPopup, setShowPopup] = useState(false);
    ...
    if (!number) {
      return (
        ...
        >
          ...
          <span className="text-[6px] text-muted-foreground mt-0.5 leading-none">
            {label}
          </span>
        </div>
      );
    ...
    return (
      ...
      >
        ...
        >
          ...
          <span
            className="text-[6px] mt-0.5 leading-none font-semibold"
          ...
          >
            ...
            <div className="flex items-center gap-1 px-3 py-1.5 overflow-x-auto scrollbar-none flex-nowrap">
              {[
                ...
                >
                  <img src={link.icon} alt="" className="w-3.5 h-3.5 rounded-sm object-contain" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
