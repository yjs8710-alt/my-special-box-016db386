import { MapPin, Phone, ChevronRight, ChevronLeft, Eye, CalendarCheck, KeyRound, StickyNote, X, ZoomIn } from "lucide-react";
    ...
    <div className="w-full">
      {revealed ? (
        <a
          ...
          className="flex items-center gap-1 w-full text-[11px] font-bold text-primary hover:underline"
        >
          <Phone className="w-3 h-3 flex-shrink-0" />
          <span className="text-[9px] text-muted-foreground font-semibold mr-0.5">{label}</span>
          {number}
        </a>
      ) : (
        <button
