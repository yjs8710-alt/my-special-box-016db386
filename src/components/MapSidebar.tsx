...
const AddressToggleCard = ({ prop, idx, buildingMemo, roomMemo, buildingPw, roomPw, regDate, chkDate, isAdmin }: AddressToggleCardProps & { isAdmin?: boolean }) => {
  ...
  const areaFromField = rawArea && /^\d+$/.test(rawArea) ? rawArea + "평" : rawArea;
  // area 필드가 비어있으면 note의 건평 값을 fallback으로 사용
  const buildingAreaFromNote = !areaFromField ? (() => {
    const m = (prop.note ?? "").match(/건평[:\s]+([^\n|]+)/);
    return m ? m[1].trim() : "";
  })() : "";
  const areaShort = areaFromField || buildingAreaFromNote;
      ...
      <div className="flex items-center gap-1 overflow-hidden flex-nowrap min-h-[22px]">
        ...
        {(() => {
          ...
          const ICON_MAP: Record<string, IconBadge> = {
            ...
            "애완동물가능":  { icon: fatDogIcon, title: "반려동물 가능", bg: "#fff7ed", color: "#c2410c", border: "#fdba74" },
            "반려동물_가능": { icon: fatDogIcon, title: "반려동물 가능", bg: "#fff7ed", color: "#c2410c", border: "#fdba74" },
            "반려동물불가":  { icon: fatDogIcon, title: "반려동물 불가", bg: "#fef2f2", color: "#b91c1c", border: "#fca5a5" },
            "애완동물불가":  { icon: fatDogIcon, title: "반려동물 불가", bg: "#fef2f2", color: "#b91c1c", border: "#fca5a5" },
