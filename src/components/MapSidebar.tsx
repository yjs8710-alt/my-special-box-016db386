...
const OptionSvgIcon = ({ name, size = 11 }: { name: string; size?: number }) => {
  ...
  const icons: Record<string, JSX.Element> = {
    ...
    "애완동물가능": <svg width={s} height={s} viewBox="0 0 24 24" fill="none"><ellipse cx="4.5" cy="10" rx="1.8" ry="2.5" stroke="currentColor" strokeWidth="1.5"/><ellipse cx="8.5" cy="7" rx="1.8" ry="2.5" stroke="currentColor" strokeWidth="1.5"/><path d="M6 13 C4 13 3 15 3 17 C3 19.5 5 21 8 21 L16 21 C19 21 21 19.5 21 17 C21 15 20 13 18 13 C16 11 14 10 12 10 C10 10 8 11 6 13Z" stroke="currentColor" strokeWidth="1.5" fill="none"/><ellipse cx="19.5" cy="10" rx="1.8" ry="2.5" stroke="currentColor" strokeWidth="1.5"/><ellipse cx="15.5" cy="7" rx="1.8" ry="2.5" stroke="currentColor" strokeWidth="1.5"/><path d="M13 17 L16 21" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/></svg>,
        ...
        {(() => {
          ...
          return (
            <span className="flex-shrink-0 flex items-center gap-0.5 whitespace-nowrap">
              ...
              {(() => {
                const bam = (prop.note ?? "").match(/건평[:\s]+([^\n|]+)/);
                const bv = bam ? bam[1].trim() : null;
                if (!bv) return null;
                return (
                  <>
                    <span className="text-[11px] mx-0.5" style={{ color: "hsl(var(--border))" }}>·</span>
                    <span className="text-[10px] font-bold" style={{ color: "hsl(var(--muted-foreground))" }}>건평</span>
                    <span className="text-[11px] font-extrabold" style={{ color: "hsl(25 90% 40%)" }}>{bv}</span>
                  </>
                );
              })()}
            </span>
              ...
              <div className="flex items-center gap-2 flex-1 min-w-0">
                <div className="min-w-0">
                  ...
                  <p className="text-[9px] text-muted-foreground mt-0.5">
                    {checkedIds.size > 0 ? `${checkedIds.size}개 선택됨` : pinnedAddress && (!pinnedIds || pinnedIds.length === 0) ? "핀 클릭 필터 중" : ""}
                  </p>
                </div>
              </div>
            </div>
