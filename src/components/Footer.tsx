    ...
    <footer className="text-foreground">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-8">
          <div>
            <div className="flex items-center gap-2 mb-3">
              <img src={logoImg} alt="집다 로고" className="h-8 object-contain" />
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-8 text-sm">
            {[
              ...
              <div key={col.title}>
                <h4 className="font-semibold text-foreground/80 mb-3">{col.title}</h4>
        ...
        <div className="border-t border-border mt-8 pt-6 flex flex-col sm:flex-row justify-between items-center gap-2">
          <p className="text-xs text-muted-foreground">© 2024 집다. All rights reserved.</p>
          <p className="text-xs text-muted-foreground">사업자등록번호: 123-45-67890 | 대표: 홍길동</p>
        </div>
      </div>
    </footer>
  );
