        ...
        <div className="flex items-center gap-2">
          ...
          >
            <div className="relative">
              ...
              >
                <span>{currentCategory ? currentCategory.short : "전체"}</span>
                <svg
                  width="10" height="6" viewBox="0 0 10 6" fill="none"
                  className="flex-shrink-0 transition-transform"
                  style={{ transform: showCategoryDrop ? "rotate(180deg)" : "rotate(0deg)" }}
                >
              ...
              {showCategoryDrop && (
                <>
                  ...
                  <div
                    className="absolute left-0 top-[calc(100%+8px)] z-[1101] rounded-2xl overflow-hidden"
                    style={{
                      minWidth: 260,
                  ...
                  >
                    ...
                    <button
                      ...
                      style={!currentCategory ? { background: "hsl(var(--primary)/0.08)" } : {}}
                    >
                      <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 text-base"
                        style={{ background: !currentCategory ? "hsl(var(--primary))" : "hsl(var(--muted))" }}>
                        <span style={{ color: !currentCategory ? "#fff" : "hsl(var(--muted-foreground))" }}>🗺</span>
                      </div>
                      <div className="text-left flex-1">
                        <p className="text-[12px] font-bold" style={{ color: "hsl(var(--foreground))" }}>전체</p>
                        <p className="text-[10px]" style={{ color: "hsl(var(--muted-foreground))" }}>모든 매물 유형</p>
                      </div>
                      {!currentCategory && (
                        <svg width="14" height="14" viewBox="0 0 14 14" fill="none" className="flex-shrink-0">
                          <path d="M2 7l3.5 3.5L12 3" stroke="hsl(var(--primary))" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                        </svg>
                      )}
                    </button>
                    ...
                    {[
                      ...
                      { ...SEARCH_CATEGORIES[2], icon: "🏢" },
                      { ...SEARCH_CATEGORIES[3], icon: "🌾" },
                    ].map((cat) => {
                      const isActive = currentCategory?.value === cat.value;
                      return (
                        <button
                          key={cat.value}
                          onClick={() => { setSelectedCategory(cat); setShowCategoryDrop(false); navigate(cat.route); }}
                          className="w-full flex items-center gap-3 px-4 py-3 border-b border-border/50 last:border-b-0 transition-all hover:bg-primary/5"
                          style={isActive ? { background: "hsl(var(--primary)/0.08)" } : {}}
                        >
                          <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 text-base"
                            style={{ background: isActive ? "hsl(var(--primary))" : "hsl(var(--muted))" }}>
                            <span style={{ color: isActive ? "#fff" : "hsl(var(--muted-foreground))" }}>{cat.icon}</span>
                          </div>
                          <div className="text-left flex-1">
                            <p className="text-[12px] font-bold" style={{ color: "hsl(var(--foreground))" }}>{cat.label}</p>
                            <p className="text-[10px]" style={{ color: "hsl(var(--muted-foreground))" }}>{cat.desc}</p>
                          </div>
                          {isActive && (
                            <svg width="14" height="14" viewBox="0 0 14 14" fill="none" className="flex-shrink-0">
                              <path d="M2 7l3.5 3.5L12 3" stroke="hsl(var(--primary))" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
